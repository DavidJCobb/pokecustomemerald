...
void LuUI_DrawWindowFrame(
   ...
   const u8 l = inner_x - 1;
   const u8 r = inner_x + inner_w;
   const u8 t = inner_y - 1;
   const u8 b = inner_y + inner_h;
   
   // C: corner
   // E: edge
   const u16 tile_c_tl = dialog_frame_first_tile_id;
   const u16 tile_e_t  = dialog_frame_first_tile_id + 1;
   const u16 tile_c_tr = dialog_frame_first_tile_id + 2;
   const u16 tile_e_l  = dialog_frame_first_tile_id + 3;
   const u16 tile_mid  = dialog_frame_first_tile_id + 4;
   const u16 tile_e_r  = dialog_frame_first_tile_id + 5;
   const u16 tile_c_bl = dialog_frame_first_tile_id + 6;
   const u16 tile_e_b  = dialog_frame_first_tile_id + 7;
   const u16 tile_c_br = dialog_frame_first_tile_id + 8;
