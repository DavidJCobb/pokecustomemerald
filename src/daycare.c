...
static void ApplyDaycareExperience(struct Pokemon *mon)
{
    ...
    for (i = 0; i < MAX_LEVEL; i++)
    {
        ...
        {
            if (gCustomGameOptions.daycare_eggs.can_teach_moves) {
...
static u16 TakeSelectedPokemonFromDaycare(struct DaycareMon *daycareMon)
{
    ...
    {
        experience = GetMonData(&pokemon, MON_DATA_EXP) + (daycareMon->steps * gCustomGameOptions.daycare_eggs.scale_step_exp);
...
static u8 GetLevelAfterDaycareSteps(struct BoxPokemon *mon, u32 steps)
{
    ...
    u32 experience = GetBoxMonData(mon, MON_DATA_EXP) + (steps * gCustomGameOptions.daycare_eggs.scale_step_exp);
...
static u32 GetDaycareCostForSelectedMon(struct DaycareMon *daycareMon)
{
    ...
    cost = ApplyCustomGameScale_u32(cost, gCustomGameOptions.daycare_eggs.scale_step_exp);
