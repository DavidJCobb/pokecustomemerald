...
| :- | :-: | :- |
| `gBattlerInMenuId` | battle controllers | Set by the player's (and cutscene-Wally's) battle controller when opening a menu, and by the `useitemonopponent` script command when NPCs use an item. Many of the functions that handle item usage rely on this value to know what Pokemon the item is being used on, when items are used during a battle. |
...
| `gCurrentActionFuncId` | battle engine | <p>Latent state for `RunTurnActionsFunctions` and the action functions (`HandleAction_...`) it invokes. Controls *which* action function it'll invoke.</p><p>The action functions are themselves latent, and it's possible for one action function to fall through into another. That fallthrough is implemented by having the action function overwrite this variable, so that a different action function is called from the next frame onward.</p> |
| `gCurrentTurnActionNumber` | battle engine | Latent state for `RunTurnActionsFunctions` and the action functions (`HandleAction_...`) it invokes. Indicates which action in the current turn is being executed; when the value becomes greater than or equal to `gBattlersCount`, all actions have completed. |
| `gDoingBattleAnim` | battle controllers | One of two pieces of latent state used by battle controllers to know when a "special" animation is playing. This is sometimes set by the battle controllers before playing the animation, and sometimes set by the animations themselves. |
| `gPlayerDpadHoldFrames` | battle controllers | <p>Latent state (a counter) for the player battle controller's input handling. Used to activate a battle UI feature wherein if "L=A" button mode is enabled, and the player holds down any D-Pad button for a long time, then the game acts like the player pressed is mashing B.</p><p>The feature is implemented improperly. It's meant to kick in after a full second of holding down any D-Pad button, but the input is checked wrong. During action selection, this counter is reset if `JOY_REPEAT(DPAD_ANY)` (D-Pad repeating or newly pressed) is false, and incremented otherwise; but key repetition only starts after 40 frames and only occurs at 5-frame intervals thereafter, so the only way for this condition to be met *every frame* is if the player presses a new D-Pad button every frame. Sure enough, holding the D-Pad while in the action menu for your second Pokemon (in a Double Battle) has no effect, failing to cancel you back to the first Pokemon.</p><p>This mistake is *not* made during target selection, for single-target moves in a Double Battle. There, `JOY_HELD(DPAD_ANY)` is checked, so the feature behaves as designed for that selection alone: holding any D-Pad button for a full second cancels target selection and returns you to move selection.</p> |
...
| `gRandomTurnNumber` | <p>Randomized at the start of each turn, and used as the dice roll to compute whether Quick Claw activates. This isn't used for any other purpose; the function that computes turn order calls `Random()` directly for the (rare) situations it needs other randomness.</p><p>This has the effect that the same `gRandomTurnNumber` value is used for all battlers in a turn, so if multiple battlers are holding Quick Claw, then their items will activate exactly in synch. The precise effect of Quick Claw, upon activation, is to make the game behave as though the wielder has an infinite Speed stat, so this means that when two opposing Pokemon are holding Quick Claw, there's a 20% chance each turn that they'll be considered to have equal Speed to each other and greater Speed than anyone else (i.e. if it's a Double Battle), and if they use equal-priority moves during that turn, then a `Random()` call will be used to break the Speed tie and decide who strikes first.</p><p>This design feels very specific and very deliberate, so it may be more appropriate to name this variable something like `gQuickClawDiceRoll`.</p> |
...
| 0x08 | `multihitString[6]` | Storage buffer, for stringifying the number of times a multi-hit move lands. This is done ASAP by `initmultihitstring` after computing the number of hits; then, that number is used as a loop counter and decremented as each hit is processed. Afterwards, the stringified number is copied from here to `B_BUFF_1` for display. |
...
| 0x0F | `twoTurnsMoveStringId` | Used by `BattleScriptFirstChargingTurn`, with the intention that it be initialized (by scripts that jump there) to whatever multi-string index a two-turn charging move (e.g. Fly, Dig, Solar Beam) should use to print the charge-up message. |
| 0x10 | `animArg1` |
| 0x11 | `animArg2` |
...
| -: | :- | :- |
| 0 | `MULTIUSE_STATE` | Several latent functions use this as a state variable. This includes the following script commands: `displaydexinfo`; `trydobeatup`; `trygivecaughtmonnick`; `updatebattlermoves`; `yesnobox`. Additionally, some scripts, such as `BattleScript_DamagingWeatherContinues`, use this as a scratch variable (e.g. a loop index), and some native processes (particularly in battle startup) use it as a latent state variable too.< |
| 0 | *unnamed* | Within `HandleTurnActionSelectionState`, the action state of battler 0. Initialized to 0 by `BattleTurnPassed`. |
| 0 | *unnamed* | The `switchhandleorder` script command takes the party index of the Pokemon to switch in, and stores it here and elsewhere. The `BattleScript_FaintedMonTryChoose` battle script checks the value to see if the player declined to switch in a Pokemon (i.e. if the value equals `PARTY_SIZE`). |
| 0 | *unnamed* | The `VARIOUS_IS_RUNNING_IMPOSSIBLE` and `VARIOUS_GET_BATTLER_FAINTED` script commands write their result here. |
| 0 | *unnamed* | The `VARIOUS_PALACE_FLAVOR_TEXT` script command writes a bool here to indicate whether a message should be displayed, along with setting `MULTISTRING_CHOOSER`. |
| 0 | *unnamed* | The `VARIOUS_ARENA_JUDGMENT_WINDOW` script command uses this as a latent state enum. The script command passes a pointer to this value into the relevant code; however, callbacks for sprites related to the Battle Arena judgment window will reference this value directly rather than through that pointer. This could be considered a subset of `MULTIUSE_STATE`, then. |
...
This object and its contents could perhaps be expressed as a jumble of structs and unions, assuming no internal padding:
